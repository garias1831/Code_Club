...
def extract_subdirectory_values(directory):
    ...
    for subdirectory in subdirectories:
        ...
        get_flagged_directories(subdirectory, required_disk_space)

def determine_unused_space(total_space, minimum_space, current_space):
    unused_disk_space = total_space - current_space
    required_disk_space = minimum_space - unused_disk_space
    return required_disk_space

def get_flagged_directories(directory, required_space):
    if directory.value >= required_space:
        flagged_directories.append(directory.value)
...
current_disk_space = main.value
required_disk_space = determine_unused_space(total_disk_space, minimum_disk_space, current_disk_space)

extract_subdirectory_values(main)

smallest_directory = min(flagged_directories)
#print(required_disk_space)
#total = sum(flagged_directories)
print(required_disk_space)
print(f'The smallest directory to delete has size: {smallest_directory}')